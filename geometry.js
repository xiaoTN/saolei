...
// 每个五边形有 5 个邻居
//
// Cairo 五边形的几何特征（Type 4，与 snub square tiling 对偶的形式）：
// - 角度序列：120°, 120°, 90°, 120°, 90°（两个直角不相邻）
// - 四条等长边和一条短边
//
// 关键：相邻五边形的边必须重合！
// 使用正方形网格，每个单元格包含一个五边形，台阶边连接到对角邻居
...
function cairoVertices(row, col) {
    ...
    // Cairo 镶嵌的正确构造：
    // 每个五边形的台阶边与对角邻居的台阶边重合
    //
    // 类型 A 的台阶边：从 (x0+s, y0+s/2) 到 (x0+s/2, y0+s)
    // 类型 B 的台阶边：从 (x0+s/2, y0) 到 (x0, y0+s/2)
    //
    // 当类型 A 在 (row, col)，类型 B 在 (row+1, col+1) 时：
    // 类型 B 的全局台阶边：从 (col+1+s/2, row+1) 到 (col+1, row+1+s/2)
    //                      = (col+1.5, row+1) 到 (col+1, row+1.5)
    // 类型 A 的全局台阶边：从 (col+1, row+0.5) 到 (col+0.5, row+1)
    // 这两条边不重合！
    //
    // 正确的构造：台阶边应该共享一个端点
    // 类型 A 的台阶边终点 (x0+s/2, y0+s) 应该等于
    // 类型 B（在 row+1, col+1）的台阶边起点 (x0'+s/2, y0')
    // 即 (col+0.5, row+1) = (col+1+0.5, row+1)，这不对！
    //
    // 重新设计：让台阶边从单元格边缘的中点开始
    // 类型 A：台阶从右中 (x0+s, y0+s/2) 到下中 (x0+s/2, y0+s)
    // 类型 B：台阶从上中 (x0+s/2, y0) 到左中 (x0, y0+s/2)
    //
    // 对于相邻的类型 A 和类型 B，它们共享单元格的边
    // 类型 A 的右边界与右侧类型 B 的左边界重合
    // 类型 A 的下边界与下方类型 B 的上边界重合
    // 但台阶边不重合，而是形成网格

    if (type === 'a') {
        // 类型 A：台阶在右下角
        // 顶点：左上角 → 右上角 → 右中（台阶起点）→ 下中（台阶终点）→ 左下角
        return [
            [x0, y0],           // 左上角 (90°)
            [x0 + s, y0],       // 右上角 (120°)
            [x0 + s, y0 + s/2], // 右中 (台阶起点)
            [x0 + s/2, y0 + s], // 下中 (台阶终点，90°)
            [x0, y0 + s],       // 左下角 (120°)
        ];
    } else {
        // 类型 B：台阶在左上角
        // 顶点：上中（台阶起点）→ 右上角 → 右下角 → 左下角 → 左中（台阶终点）
        return [
            [x0 + s/2, y0],     // 上中 (台阶起点，90°)
            [x0 + s, y0],       // 右上角 (120°)
            [x0 + s, y0 + s],   // 右下角 (120°)
            [x0, y0 + s],       // 左下角 (90°)
            [x0, y0 + s/2],     // 左中 (台阶终点)
...
function cairoNeighbors(row, col) {
    ...
    // 第 5 个邻居：对角方向
    // 类型 A 的台阶边连接到右下对角的类型 B
    // 类型 B 的台阶边连接到左上对角的类型 A
