...
const _mpRecentLocalReveals = {}; // key → timestamp，记录本端最近的 reveal 操作（用于冲突检测）
...
function backToMenu() {
    ...
    for (const k in _mpRecentLocalReveals) delete _mpRecentLocalReveals[k];
    showScreen('startScreen');
    refreshRoomList();
}
    ...
    if (MP.isMultiplayer()) {
        MP.onMessage = (msg) => {
            ...
            if (type === 'reveal') {
                ...
                // conflict detection: if local also revealed this key within 1s
                const _localRevealTs = _mpRecentLocalReveals[key];
                if (_localRevealTs && Date.now() - _localRevealTs < 1000) {
                    delete _mpRecentLocalReveals[key];
                    showToast('对方抢先一步！', 2000);
                }
                if (board[key] === -1) {
                    // 对方踩雷，显示失败界面
...
function handleRightClick(e, row, col, opts = {}) {
    ...
    if (MP.isMultiplayer() && !opts.fromRemote) {
        _mpRecentLocalReveals[key] = Date.now();
    }
